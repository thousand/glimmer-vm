...
class SyntaxErrors extends RenderTest {
  ...
  'context switching using ../ is not allowed'() {
    this.assert.throws(() => {
      preprocess('<div><p>{{../value}}</p></div>', undefined, { moduleName: 'test-module' });
  ...
  'mixing . and / is not allowed'() {
    this.assert.throws(() => {
      preprocess('<div><p>{{a/b.c}}</p></div>', undefined, { moduleName: 'test-module' });
  ...
  'Block params in HTML syntax - Throws exception if given zero parameters'() {
    this.assert.throws(() => {
      preprocess('<x-bar as ||>foo</x-bar>', undefined, { moduleName: 'test-module' });
  ...
  'Block params in HTML syntax - Throws an error on invalid block params syntax'() {
    this.assert.throws(() => {
      preprocess('<x-bar as |x y>{{x}},{{y}}</x-bar>', undefined, { moduleName: 'test-module' });
  ...
  'Block params in HTML syntax - Throws an error on invalid identifiers for params'() {
    this.assert.throws(() => {
      preprocess('<x-bar as |x foo.bar|></x-bar>', undefined, { moduleName: 'test-module' });
    }, syntaxErrorFor("Invalid identifier for block parameters, 'foo.bar'", '<x-bar as |x foo.bar|></x-bar>', 'test-module', 1, 0));

    this.assert.throws(() => {
      preprocess('<x-bar as |x "foo"|></x-bar>', undefined, { moduleName: 'test-module' });
    }, syntaxErrorFor('" is not a valid character within attribute names', '', 'test-module', 1, 17));

    this.assert.throws(() => {
      preprocess('<x-bar as |foo[bar]|></x-bar>', undefined, { moduleName: 'test-module' });
