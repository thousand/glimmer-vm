...
class CompileErrorTests extends RenderTest {
  ...
  'A helpful error message is provided for unmatched end tags'() {
    this.assert.throws(() => {
      preprocess('</p>', undefined, { moduleName: 'test-module' });
  ...
  'A helpful error message is provided for end tags with attributes'() {
    this.assert.throws(() => {
      preprocess('<div>\nSomething\n\n</div foo="bar">', undefined, { moduleName: 'test-module' });
  ...
  'error line numbers include multiple mustache lines'() {
    this.assert.throws(() => {
      preprocess('<div>\n<p>\n{{some-comment}}</div>{{some-comment}}', undefined, {
        moduleName: 'test-module',
      });
  ...
  'Unquoted attribute with expression throws an exception'() {
    this.assert.throws(
      () => preprocess('<img class=foo{{bar}}>', undefined, { moduleName: 'test-module' }),
    ...
    this.assert.throws(
      () => preprocess('<img \nclass={{foo}}bar>', undefined, { moduleName: 'test-module' }),
      expectedError('class={{foo}}bar', 2, 0)
    );
    this.assert.throws(
      () =>
        preprocess('<div \nclass\n=\n{{foo}}&amp;bar ></div>', undefined, {
          moduleName: 'test-module',
        }),
      expectedError('class\n=\n{{foo}}&amp;bar', 2, 0)
    );
